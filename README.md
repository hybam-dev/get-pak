...
Each ROI report is one XLSX workbook with exactly two worksheets: Water quality (first and active) and Processing details. Water quality begins with compact text record_id and one native UTC acquisition datetime; Processing details retains record_id, scene_uid, acquisition datetime, and separate acquisition date/time fields. Water-quality statistics use physical units and retain valid-pixel counts and quality indicators; requested Rrs statistics follow the primary products. Processing details retain source/mask paths, processor and platform metadata, encoding factors and fallback sources, per-band diagnostics, and outcomes. Water quality freezes at C2 with 45-degree measurement headers; Processing details freezes at F2.
